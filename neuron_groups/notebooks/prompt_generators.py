...
class ExplanationPromptGen:
    ...
    def __post_init__(self):
        self.attn_head = f'This attention head in particular helps to predict that the last token is "{self.token}", but it is only active in some documents and not others.\n'
...
class IterationPromptGen:
    ...
    def get_str_from_categories(self, active_examples, inactive_examples):
        newline = "\n"
        active_examples = [active_example + self.token for active_example in active_examples]
        inactive_examples = [active_example + self.token for active_example in inactive_examples]
